...
sio_lobby = socketio.Client()
sio_game  = socketio.Client()
...
def mirror_and_rotate_obstacle_shapely(obstacle, base_fence, target_fence, angle_deg=60):
    ...
    for wp in obs_wps:
        ...
        # Ojo: escalamos usando target_bbox, pero la lon va "al revés"
...
def onSessionSelected(session_id):
    global currentSessionId
    currentSessionId = session_id

    # Conecta al namespace /lobby y únete a la sala
    if not sio_lobby.connected:
        sio_lobby.connect('http://localhost:9000', namespaces=['/lobby'])
    sio_lobby.emit('join_lobby', {'sessionId': currentSessionId}, namespace='/lobby')

    # Conecta al namespace /game (el servidor hará join_room con el auth)
    if not sio_game.connected:
        sio_game.connect(
            'http://localhost:9000',
            namespaces=['/game'],
...
def startGame():
    ...
    sio_profesor = socketio.Client()
    sio_profesor.connect('http://localhost:8000', namespaces=['/jocs'])
    sio_profesor.emit('start_game', {}, namespace='/jocs')
...
def processTelemetryInfo(id, telemetry_info):
    ...
    state = {
      ...
      'speed': telemetry_info.get('groundSpeed'),
    }
    sio_game.emit(
      'state_update',
